...
export default function App() {
  ...
  const handleDeptSelect = useCallback((dept: Department) => {
    ...
    setSelectedSubject(null);
    setIsBookmarksView(false);
    window.scrollTo({ top: 0, behavior: 'instant' }); // Faster than smooth for performance feel
  }, [pushHistory]);
  ...
  return (
    <div className="relative min-h-screen flex flex-col font-sans">
      ...
      <footer className="relative mt-8 p-12 text-center text-xs text-slate-500">
        <p>© {new Date().getFullYear()} PolyStudy. Fast, Open, Educational.</p>
      </footer>
    </div>
  );
}
